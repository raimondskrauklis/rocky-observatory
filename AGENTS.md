...
| **Review context SSOT** | [.revy/review-context.json](.revy/review-context.json) — Revy inject; mirror [.agent/review-context.json](.agent/review-context.json) |
...
**LOOP contract:** The agent **executes until the program is done**. Do **not** ask “Continue?”. Local commit every phase. Honour `hosting.kind` — GitHub `raimondskrauklis/rocky-observatory`; Revy on PRs (same production App as `raimondskrauklis/revy`). Never push while Revy is `pending` / `in_progress` / `queued`. **Only pause:** migration subphase.
...
Not installed: `sentry-fix-issues` (no Sentry MCP yet), `staging-validation`.
...
**Scope:** manifest default is `docs/**` until application trees exist. Widen in `.revy/review-context.json` `programs[].scope` (then copy to `.agent/`) when a program needs `.agent/**`, `.cursor/**`, `apps/**`, or `infrastructure/**`.
...
Program state: bootstrap **done**; Revy reviewer **on** (GitHub App still needs this repo added — [docs/runbooks/github-revy.md](docs/runbooks/github-revy.md)); DevOps at *general plan + operations* — next is `create-execution-plan` for P0–P6.
